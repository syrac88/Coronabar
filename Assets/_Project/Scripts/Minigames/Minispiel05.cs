...
/// <summary>
/// Minispiel 05 – Mathe-Duell: Rechnen unter Zeitdruck (+1 richtig, -1 falsch).
/// Spiel-UI (Aufgabe + Buttons) wird zur Laufzeit im Spielbereich aufgebaut.
/// Score-Anzeige liegt im Prefab-Header (textScore), verwaltet von MinigameBase.
/// </summary>
public class Minispiel05 : MinigameBase
{
    // ── Layout-Konstanten (Spielbereich, bottom-anchor) ───────────────────
    private const int   AnswerButtonCount       = 3;
    private const int   MinNumber               = 1;
    private const int   MaxNumber               = 10;
    private const float ButtonBottomOffset      = 60f;    // Pflichtabstand Rahmen unten
    ...
    private void EnsureGameUI()
    {
        ...
        if (minigamePanel == null) { Debug.LogError("[Minispiel05] minigamePanel fehlt."); return; }
        ...
        string text, bool anchorBottom)
    {
        ...
        tmp.font = font; tmp.fontSize = fontSize; tmp.alignment = align;
        tmp.color = Color.white; tmp.text = text;
        return tmp;
    }
